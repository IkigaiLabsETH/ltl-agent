...
export const opportunityProvider: Provider = {
  name: "opportunity",
  description:
    "Provides investment opportunity alerts, signals, and performance tracking",
  private: true, // Must be explicitly included
  position: 8, // Late in the chain for strategic analysis

  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    elizaLogger.debug(
      "🚨 [OpportunityProvider] Providing investment opportunity context",
    );
...
function analyzeOpportunities(activeAlerts: any[], metrics: any): any {
  ...
  if (activeAlerts?.length > 0) {
    ...
    averageConfidence =
      Math.round((totalConfidence / activeAlerts.length) * 100) / 100;
...
function categorizeAlerts(activeAlerts: any[]): any {
  ...
  if (activeAlerts?.length > 0) {
    activeAlerts.forEach((alert) => {
      ...
      // By asset category
      if (
        alert.asset?.toLowerCase().includes("bitcoin") ||
        alert.asset === "btc"
      ) {
        categories.bitcoinThesis.push(alert);
      } else if (
        ["ethereum", "solana", "sui", "ada"].includes(
          alert.asset?.toLowerCase(),
        )
      ) {
        categories.altcoin.push(alert);
      } else if (
        ["mstr", "tsla", "msty", "coin"].includes(alert.asset?.toLowerCase())
      ) {
...
function analyzeAlertPerformance(alertHistory: any[], metrics: any): any {
  ...
  if (alertHistory?.length > 0) {
    ...
    const assetStats = Object.entries(assetPerformance)
...
function buildOpportunityContext(
  ...
  context.push(
    `🎯 Market opportunity: ${opportunityAnalysis.marketOpportunity}`,
  );
  ...
  if (alertCategories.immediate.length > 0) {
    ...
    alertCategories.immediate
      .slice(0, 3)
      .forEach((alert: any, index: number) => {
