...
export const nftProvider: Provider = {
  name: "nft",
  description:
    "Provides NFT collection data, floor prices, and digital art market analysis",
...
function analyzeNFTMarketConditions(nftData: any): any {
  ...
  if (nftData?.summary) {
    const { totalVolume24h, avgFloorPrice, topPerformers, worstPerformers } =
      nftData.summary;

    // Determine sentiment based on performers ratio
    const performersRatio =
      topPerformers.length / (topPerformers.length + worstPerformers.length);
    ...
    if (avgFloorPrice > 5) {
      // High-value floor suggests premium market
...
function findStandoutNFTCollections(nftData: any): any {
  ...
  if (nftData?.collections) {
    ...
    standouts.highVolume = nftData.collections
      ...
      .sort(
        (a: any, b: any) => b.stats.one_day_volume - a.stats.one_day_volume,
      );
    ...
    standouts.priceGainers = nftData.collections
      ...
      .sort(
        (a: any, b: any) => b.stats.one_day_change - a.stats.one_day_change,
      );
    ...
    standouts.priceLosers = nftData.collections
      .filter((collection: any) => collection.stats?.one_day_change < -10)
      .sort(
        (a: any, b: any) => a.stats.one_day_change - b.stats.one_day_change,
      );
...
function analyzeNFTTrends(nftData: any): any {
  ...
  if (nftData?.summary) {
    const { topPerformers, worstPerformers, totalVolume24h, avgFloorPrice } =
      nftData.summary;
    ...
    const generativeCount =
      nftData.collections?.filter((c: any) => c.category === "generative-art")
        .length || 0;
    trends.generativeArtFocus =
      generativeCount > nftData.collections?.length * 0.5;
...
function buildNFTContext(
  ...
  if (standoutCollections.highValue.length > 0) {
    ...
    standoutCollections.highValue
      .slice(0, 3)
      .forEach((collection: any, index: number) => {
  ...
  if (standoutCollections.priceGainers.length > 0) {
    ...
    standoutCollections.priceGainers
      .slice(0, 2)
      .forEach((collection: any, index: number) => {
  ...
  if (standoutCollections.priceLosers.length > 0) {
    ...
    standoutCollections.priceLosers
      .slice(0, 2)
      .forEach((collection: any, index: number) => {
        const change = collection.stats?.one_day_change?.toFixed(1) || "N/A";
        context.push(
