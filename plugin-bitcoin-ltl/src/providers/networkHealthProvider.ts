...
export const networkHealthProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    elizaLogger.debug('🌐 [NetworkHealthProvider] Providing Bitcoin network health context');
    
    try {
      // Get the Bitcoin network data service
      const networkService = runtime.getService('bitcoin-network-data') as BitcoinNetworkDataService;
      if (!networkService) {
        elizaLogger.warn('[NetworkHealthProvider] BitcoinNetworkDataService not available');
        return {
      ...
      if (!networkData) {
        elizaLogger.debug('[NetworkHealthProvider] No network data available yet');
        return {
      ...
      elizaLogger.debug(`[NetworkHealthProvider] Providing network health context - Block: ${networkData.network.blockHeight}`);
      ...
      elizaLogger.error('[NetworkHealthProvider] Error providing network context:', error);
      return {
...
function analyzeNetworkHealth(networkData: any): any {
  ...
  if (networkData?.network) {
    ...
    if (hashRate && blockHeight) {
      if (hashRate > 5e20 && blockHeight > 800000) { // Very high hash rate
