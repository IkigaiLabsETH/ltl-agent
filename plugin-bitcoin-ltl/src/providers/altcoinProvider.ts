...
export const altcoinProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    elizaLogger.debug('🪙 [AltcoinProvider] Providing comprehensive altcoin market context');
    ...
    try {
      ...
      try {
        ...
        elizaLogger.warn('[AltcoinProvider] Service not available, using API data only');
      }
...
async function getBasicAltcoinPrices(): Promise<Record<string, CoinData>> {
  try {
    ...
    elizaLogger.error('[AltcoinProvider] Failed to fetch basic price data:', error);
    throw error;
  }
}
...
async function getTrendingCoins(): Promise<TrendingCoin[]> {
  try {
    ...
    elizaLogger.error('[AltcoinProvider] Failed to fetch trending coins:', error);
    throw error;
  }
}
...
async function getGlobalMarketData(): Promise<GlobalMarketData> {
  try {
    ...
    elizaLogger.error('[AltcoinProvider] Failed to fetch global market data:', error);
    throw error;
  }
}
...
async function getTopCoinsMarketData(): Promise<MarketDataCoin[]> {
  try {
    ...
    elizaLogger.error('[AltcoinProvider] Failed to fetch top coins market data:', error);
    throw error;
  }
}
...
function buildComprehensiveResponse(
  ...
  if (global) {
    ...
    context.push(`• Total Market Cap: $${(totalMarketCap / 1000000000000).toFixed(1)}T`);
