...
export const stockProvider: Provider = {
  name: "stock",
  description:
    "Provides stock market data, MAG7 analysis, and Bitcoin equity performance",
...
function analyzeStockMarketConditions(stockData: any, mag7: any[]): any {
  ...
  if (stockData?.performance) {
    const { mag7Average, sp500Performance, techStocksAverage } =
...
function analyzeSectorPerformance(
  ...
  if (stocks?.length > 0) {
    ...
    const sectorAverages = Object.entries(sectorPerformance)
      .map(([sector, changes]: [string, number[]]) => ({
        sector,
        average:
          changes.length > 0
    ...
    const techAvg =
      sectorAverages.find((s) => s.sector === "tech")?.average || 0;
    const btcAvg =
      sectorAverages.find((s) => s.sector === "bitcoin-related")?.average || 0;
...
function buildStockContext(
  ...
  context.push(
    `💡 Risk appetite: ${marketAnalysis.riskOn ? "Risk-ON" : "Risk-OFF"}`,
  );
  context.push("");
  ...
  if (bitcoinStockPerformers.outperformers.length > 0) {
    ...
    bitcoinStockPerformers.outperformers
      .slice(0, 3)
      .forEach((stock: any, index: number) => {
        context.push(
