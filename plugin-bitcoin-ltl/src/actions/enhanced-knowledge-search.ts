import { Action, IAgentRuntime, Memory, ModelType, State } from "@elizaos/core";
...
export const enhancedKnowledgeSearchAction: Action = {
  ...
  description:
    "Enhanced knowledge search with semantic understanding and context-aware results",
    ...
    try {
      ...
      if (!digestService) {
        ...
        if (callback) {
          await callback({
            thought:
              "The knowledge digest service is not available, so I cannot perform an enhanced search.",
