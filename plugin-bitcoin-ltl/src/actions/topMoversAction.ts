...
export const topMoversAction: Action = createActionTemplate({
  ...
  description:
    "Comprehensive analysis of top gaining and losing cryptocurrencies from the top 100 by market cap over 24 hours with sentiment analysis",
  similes: [
    "TOP_GAINERS",
    "TOP_LOSERS",
    "MARKET_MOVERS",
    ...
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User wants to see top gainers. I need to analyze the strongest performers from the top 100 crypto by market cap, identify rotation patterns, and provide market sentiment analysis while warning about volatility risks.",
    ...
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User asking about biggest losers. I should analyze the worst performers, identify if it's a general alt purge or specific sector weakness, and provide perspective on whether these are buying opportunities or continued decline.",
          actions: ["TOP_MOVERS_ACTION"],
        },
      },
    ],
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User wants comprehensive movers analysis. I should provide both gainers and losers, identify rotation patterns between different crypto sectors, and assess overall market volatility and sentiment.",
          actions: ["TOP_MOVERS_ACTION"],
        },
      },
    ],
  ...
  handlerFn: async (
    ...
    let thoughtProcess =
      "User is requesting top movers analysis. I need to analyze the biggest gainers and losers from the top 100 crypto by market cap, identify market rotation patterns, and provide sentiment analysis.";
    let queryType: "gainers" | "losers" | "both" = "both";

    if (
      messageText.includes("gainer") ||
      messageText.includes("winner") ||
      messageText.includes("pump")
    ) {
      queryType = "gainers";
      thoughtProcess =
        "User wants to see top gainers. I should focus on the strongest performers, identify which sectors are leading, and provide context about market rotation and momentum.";
    } else if (
      messageText.includes("loser") ||
      messageText.includes("dump") ||
      messageText.includes("red")
    ) {
      queryType = "losers";
      thoughtProcess =
        "User asking about biggest losers. I should analyze the worst performers, identify if it's sector-specific weakness or general market decline, and assess if these are opportunities or continued weakness.";
    }

    try {
      const realTimeDataService =
        runtime.getService<RealTimeDataService>("real-time-data");
      ...
      let errorMessage =
        "Top movers analysis systems operational. The casino continues regardless of our monitoring capabilities.";
      ...
      if (
        ...
        errorMessage =
          "Market data rate limited. CoinGecko overwhelmed with degenerates. Analysis will resume shortly.";
      } else if (
        ...
        errorMessage =
          "Market data connectivity issues. Crypto markets pump and dump independently of our monitoring infrastructure.";
      } else if (
        errorMsg.includes("service") ||
        errorMsg.includes("unavailable")
      ) {
        errorMessage =
          "Market analysis service temporarily down. Price discovery continues in the decentralized casino.";
      }
...
function formatTopMoversResponse(
  ...
  if (type === "gainers" || type === "both") {
    if (topGainers.length === 0) {
      responseText +=
        "🚨 No significant gainers in top 100 crypto today. Market bleeding or data lag. ";
    } else {
  ...
  if (type === "losers" || type === "both") {
    if (topLosers.length === 0) {
      responseText +=
        "🎯 No significant losers in top 100 crypto today. Everything pumping or data lag. ";
    } else {
