...
export const dexScreenerAction: Action = createActionTemplate({
  ...
  description:
    "Comprehensive analysis of trending and top tokens from DEXScreener with liquidity analysis for Solana gems and memecoin radar",
  similes: [
    "trending tokens",
    "dex screener",
    "dexscreener",
    ...
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User wants current DEXScreener trending data. I need to analyze trending tokens, filter by liquidity thresholds, assess market quality, and provide perspective on these speculative plays versus Bitcoin.",
    ...
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User seeking high-liquidity Solana tokens. I should filter by strict liquidity criteria, highlight the most tradeable options, and remind about the speculative nature of most DEX tokens.",
          actions: ["DEX_SCREENER_ACTION"],
        },
      },
    ],
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User asking about memecoin trends on Solana. I need to assess the current memecoin landscape, identify trending themes, and provide realistic perspective on the speculative nature of these tokens.",
          actions: ["DEX_SCREENER_ACTION"],
        },
      },
    ],
  ...
  handlerFn: async (
    ...
    const thoughtProcess =
      "User is requesting DEXScreener analysis. I need to analyze trending tokens, assess liquidity quality, identify Solana gems, and provide perspective on these speculative plays versus sound money principles.";
...
function analyzeDexData(
  ...
  const avgLiquidity =
    trendingTokens.length > 0
      ? trendingTokens.reduce((sum, t) => sum + t.totalLiquidity, 0) /
        trendingTokens.length
      : 0;
  const avgVolume =
    trendingTokens.length > 0
      ? trendingTokens.reduce((sum, t) => sum + t.totalVolume, 0) /
        trendingTokens.length
      : 0;
  ...
  if (type === "trending" || type === "both") {
    if (trendingTokens.length === 0) {
      responseText +=
        "🚨 No trending Solana tokens meet liquidity thresholds (>$100K liq, >$20K vol). Market cooling or DEX data lag. ";
    } else {
      ...
      const trendingText = topTrending
        .map((token) => {
          ...
          const ratio = token.liquidityRatio
            ? token.liquidityRatio.toFixed(2)
            : "N/A";
  ...
  if (trendingTokens.length > 5) {
    responseText +=
      "High liquidity = actual tradability. Most boosted tokens are exit liquidity for degens.";
  } else if (trendingTokens.length > 0) {
    responseText +=
      "Limited selection meeting thresholds. Quality over quantity in this market.";
  } else {
    responseText +=
      "Solana casino quiet. Bitcoin dominance continues or DEX data lag.";
  }
