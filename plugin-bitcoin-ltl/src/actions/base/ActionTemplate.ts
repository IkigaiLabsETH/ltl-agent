...
export interface ActionConfig {
  ...
  description: string;
  similes?: string[];
  validateFn: (runtime: IAgentRuntime, message: Memory, state?: State) => Promise<boolean>;
  handlerFn: (runtime: IAgentRuntime, message: Memory, state?: State, options?: any, callback?: HandlerCallback) => Promise<unknown>;
...
export const ValidationPatterns = {
  ...
  isNetworkHealthRequest: (text: string): boolean => {
    const keywords = [
      ...
      'bitcoin hashrate', 'bitcoin difficulty', 'bitcoin mempool',
      ...
      'hashrate', 'difficulty', 'mempool', 'network status'
    ];
  ...
  isKnowledgeDigestRequest: (text: string): boolean => {
    const keywords = [
      'digest', 'knowledge digest', 'daily digest', 'research summary',
      'knowledge summary', 'generate digest', 'create digest', 'research digest',
      'summarize research', 'show insights', 'what have we learned',
      'intelligence summary', 'insights digest', 'recent learnings',
      'knowledge synthesis', 'research intelligence', 'intelligence digest'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isOpportunityAlertsRequest: (text: string): boolean => {
    const keywords = [
      'alerts', 'opportunities', 'opportunity alerts', 'check alerts',
      'show alerts', 'any alerts', 'investment alerts', 'market alerts',
      'what opportunities', 'any opportunities', 'signals', 'market signals',
      'investment signals', 'what should i watch', 'watchlist',
      'immediate opportunities', 'active alerts', 'current opportunities'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isAltcoinRequest: (text: string): boolean => {
    const keywords = [
      'altcoin', 'altcoins', 'eth', 'ethereum', 'solana', 'sol', 'sui', 
      'hyperliquid', 'hype', 'chainlink', 'link', 'uniswap', 'uni', 'aave',
      'ondo', 'ethena', 'ena', 'berachain', 'bera', 'avalanche', 'avax',
      'stacks', 'stx', 'dogecoin', 'doge', 'pepe', 'mog', 'bittensor', 'tao',
      'render', 'rndr', 'fartcoin', 'railgun', 'portfolio', 'curated',
      'performance', 'gains', 'pumping', 'mooning', 'defi', 'memecoins'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isTopMoversRequest: (text: string): boolean => {
    const keywords = [
      'top gainers', 'top losers', 'biggest movers', 'market winners', 
      'market losers', 'daily gainers', 'daily losers', 'crypto winners',
      'crypto losers', 'best performers', 'worst performers', 'pumping coins',
      'dumping coins', 'green coins', 'red coins', 'market movers',
      'gainers', 'losers', 'movers', 'pumping', 'dumping', 'winners', 'losers'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isBtcRelativePerformanceRequest: (text: string): boolean => {
    const keywords = [
      'outperforming', 'outperform', 'vs btc', 'vs bitcoin', 'altcoins',
      'altcoin', 'beating bitcoin', 'beat bitcoin', 'relative performance',
      'performance vs bitcoin', 'which coins', 'top performers',
  ...
  isNFTRequest: (text: string): boolean => {
    const keywords = [
      ...
      'archetype', 'terraforms', 'meridian', 'sightseers', 'progression'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isDexScreenerRequest: (text: string): boolean => {
    const keywords = [
      'trending tokens', 'dex screener', 'dexscreener', 'top tokens',
      'solana gems', 'new tokens', 'boosted tokens', 'trending solana',
      'dex trends', 'token discovery', 'memecoin radar', 'solana trending',
      'hot tokens', 'liquid tokens', 'token screener'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isETFRequest: (text: string): boolean => {
    const keywords = [
      ...
      'ibit', 'fbtc', 'arkb', 'bitb', 'gbtc', 'hodl', 'ezbc', 'brrr', 'btco'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isTop100VsBtcRequest: (text: string): boolean => {
    const keywords = [
      'top 100', 'altcoins vs bitcoin', 'outperforming bitcoin',
      'underperforming bitcoin', 'bitcoin dominance', 'altcoin performance',
      'btc pairs', 'altseason', 'bitcoin relative performance',
      'crypto vs bitcoin', 'outperformers', 'underperformers'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isTrendingCoinsRequest: (text: string): boolean => {
    const keywords = [
      'trending', 'trending crypto', 'trending coins', 'hot coins',
      'whats trending', 'what is trending', 'popular coins', 'viral coins',
      'buzz coins', 'hype coins', 'social trending', 'most searched',
      'community favorites', 'trending altcoins', 'hottest coins'
    ];
    return keywords.some(keyword => text.includes(keyword));
  },
  ...
  isBookingOptimizationRequest: (text: string): boolean => {
    const keywords = [
      ...
      'smart booking', 'strategy', 'value', 'worth it', 'hotel comparison'
    ];
