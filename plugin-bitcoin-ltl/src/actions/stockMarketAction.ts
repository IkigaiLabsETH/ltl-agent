 ...
 */
function formatStockAnalysis(stocks: any[], mag7: any[], performance: any): string {
  ...
  if (performance.topPerformers.length > 0) {
    const topPerformersText = performance.topPerformers.slice(0, 3).map((comp: any) => {
  ...
  // MAG7 breakdown
  if (mag7.length > 0) {
    const mag7Text = mag7.slice(0, 3).map(stock => 
