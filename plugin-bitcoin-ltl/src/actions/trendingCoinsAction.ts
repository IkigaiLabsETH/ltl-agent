...
export const trendingCoinsAction: Action = createActionTemplate({
  ...
  description:
    "Analysis of trending cryptocurrencies based on CoinGecko search activity and community interest with market sentiment assessment",
  similes: [
    "trending",
    "trending crypto",
    "trending coins",
    ...
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User wants current trending cryptocurrency data. I need to analyze trending coins, categorize them by narrative, assess market sentiment, and provide perspective on trend-following versus sound money principles.",
    ...
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User asking for hot/trending coins. I should identify the current trending tokens, categorize by sector, assess if this represents quality rotation or speculation, and maintain Bitcoin perspective.",
          actions: ["TRENDING_COINS_ACTION"],
        },
      },
    ],
    [
      ...
      },
      {
        ...
        content: {
          ...
          thought:
            "User interested in crypto conversation topics. I should analyze trending coins to identify dominant narratives, assess community sentiment, and provide perspective on trend-chasing behavior.",
          actions: ["TRENDING_COINS_ACTION"],
        },
      },
    ],
  ...
  handlerFn: async (
    ...
    const thoughtProcess =
      "User is requesting trending cryptocurrency analysis. I need to analyze community search activity, categorize trending narratives, assess market sentiment, and provide perspective on trend-following versus sound money principles.";
...
function analyzeTrendingCoins(
  ...
  const memeCoins = coins.filter((coin) =>
    ["PEPE", "DOGE", "SHIB", "BONK", "WIF", "FLOKI", "NEIRO", "MOG"].includes(
      coin.symbol.toUpperCase(),
    ),
  );
  const aiCoins = coins.filter((coin) =>
    ["TAO", "FET", "RNDR", "OCEAN", "AGIX", "WLD", "HYPE", "THETA"].includes(
      coin.symbol.toUpperCase(),
    ),
  );
  ...
  if (memeCoins.length >= 3) {
    ...
    responseText +=
      "Mixed trending signals. No clear narrative dominance. Stay focused on fundamentals.";
  }

  // Add Bitcoin perspective
  if (narrativeFocus === "meme") {
    responseText +=
      " Remember - trending means exit liquidity for early movers. Bitcoin remains the only asset with no marketing department.";
  } else if (narrativeFocus === "ai") {
    responseText +=
      " Trend following is wealth following, not wealth creating. Bitcoin needs no narrative beyond sound money.";
  } else {
    responseText +=
      " Community attention shifts rapidly. Bitcoin's fundamentals remain constant.";
  }
