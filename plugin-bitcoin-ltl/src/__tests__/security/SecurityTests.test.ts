...
describe('Security Tests', () => {
  ...
  describe('Input Validation Security', () => {
    ...
    it('should prevent SQL injection in cache keys', async () => {
      ...
      for (const maliciousKey of maliciousKeys) {
        ...
        await expect(cacheService.set(maliciousKey, 'test-value', 60000)).resolves.not.toThrow();
  ...
  describe('Data Protection Security', () => {
    ...
    it('should prevent data leakage through error messages', async () => {
      ...
      vi.spyOn(cacheService, 'set').mockRejectedValue(new Error(`Failed to store: ${JSON.stringify(sensitiveData)}`));

      // Should handle error without exposing sensitive data
      await expect(cacheService.set('test', sensitiveData, 60000)).rejects.toThrow();
  ...
  describe('Rate Limiting Security', () => {
    it('should prevent brute force attacks on configuration', async () => {
      const maliciousValues = Array(1000).fill(null).map((_, i) => `malicious-value-${i}`);
    ...
    it('should handle cache flooding attacks', async () => {
      const floodKeys = Array(1000).fill(null).map((_, i) => `flood-key-${i}`);

      // Attempt to flood cache
      for (const key of floodKeys) {
        await expect(cacheService.set(key, 'flood-value', 60000)).resolves.not.toThrow();
      }
    ...
    it('should prevent memory exhaustion attacks', async () => {
      ...
      const largeKeys = Array(100).fill(null).map((_, i) => `large-key-${i}`);

      // Attempt to store large amounts of data
      for (const key of largeKeys) {
        await expect(cacheService.set(key, largeData, 60000)).resolves.not.toThrow();
      }
  ...
  describe('Error Handling Security', () => {
    it('should not expose internal errors', async () => {
      ...
      vi.spyOn(configService, 'get').mockImplementation(() => {
        throw new Error('Internal database error: connection failed to 192.168.1.100:5432');
      });
  ...
  describe('Resource Security', () => {
    it('should prevent resource exhaustion', async () => {
      ...
      const largeObjects = Array(1000).fill(null).map(() => ({
      ...
      for (const obj of largeObjects) {
        await expect(cacheService.set(`large-${obj.id}`, obj, 60000)).resolves.not.toThrow();
      }
  ...
  describe('Compliance & Audit Security', () => {
    it('should maintain audit trail for sensitive operations', async () => {
      const sensitiveOperations = [
        { action: 'config.set', key: 'apis.coingecko.apiKey', value: 'new-key' },
        { action: 'config.set', key: 'security.masterKey', value: 'master-key' },
