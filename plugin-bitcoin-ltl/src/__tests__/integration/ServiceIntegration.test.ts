...
describe("Service Integration Tests", () => {
  ...
  describe("Error Handling Integration", () => {
    it("should propagate errors through the service chain", async () => {
      ...
      vi.spyOn(bitcoinNetworkService, "updateData").mockRejectedValue(
        new Error("Bitcoin Service Error"),
      );
      vi.spyOn(marketDataService, "updateData").mockRejectedValue(
    ...
    it("should recover from transient failures", async () => {
      ...
      let callCount = 0;
      vi.spyOn(bitcoinNetworkService, "updateData").mockImplementation(
        async () => {
          callCount++;
          if (callCount === 1) {
      ...
      await expect(bitcoinNetworkService.updateData()).rejects.toThrow(
        "Transient Error",
      );
  ...
  describe("Configuration Integration", () => {
    it("should use centralized configuration across all services", async () => {
      ...
      const bitcoinInterval = configService.get(
        "services.bitcoinNetwork.updateInterval",
      );
      const marketInterval = configService.get(
