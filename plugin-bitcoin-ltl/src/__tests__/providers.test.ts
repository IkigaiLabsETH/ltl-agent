    ...
    switch (serviceName) {
      case 'bitcoin-data':
        return {
          ...
          calculateFreedomMathematics: vi.fn().mockResolvedValue({
            btcNeeded: 100,
            currentPrice: 100000,
            scenarios: { conservative: { price: 200000, btc: 50, timeline: '5 years' } },
      ...
      case 'real-time-data':
        return {
          ...
          getTopMoversData: vi.fn().mockReturnValue({
            topGainers: [{ symbol: 'DOGE', price_change_percentage_24h: 15.5 }],
          ...
          getNewsItems: vi.fn().mockReturnValue([
            { title: 'Bitcoin hits new high', sentiment: 'positive', publishedAt: new Date() }
          ]),
      ...
      case 'travel-data':
        return {
          getTravelData: vi.fn().mockReturnValue({
            hotels: [
              { hotelId: 'biarritz_palace', name: 'Hôtel du Palais', city: 'biarritz' },
              { hotelId: 'bordeaux_grand', name: 'Grand Hôtel Bordeaux', city: 'bordeaux' }
            ],
            currentRates: [
              { hotelId: 'biarritz_palace', totalPrice: 500, basePrice: 450, currency: 'EUR' }
            ],
            optimalBookingWindows: [
              { 
                ...
                city: 'biarritz',
                bestDates: [
                  { checkIn: '2024-03-15', checkOut: '2024-03-18', totalPrice: 450, savings: 50, savingsPercentage: 10 }
                ],
                seasonalAnalysis: { season: 'mid', averagePrice: 500, demandLevel: 'moderate' }
              }
            ],
            lastUpdated: new Date()
          }),
          getTravelInsights: vi.fn().mockReturnValue({
            cityAnalysis: [
              { city: 'biarritz', bestMonths: [3, 4, 5], averageSavings: 15, optimalStayLength: 3 },
              { city: 'bordeaux', bestMonths: [3, 4, 5], averageSavings: 12, optimalStayLength: 2 }
            ],
            pricePatterns: [
              { month: 3, monthName: 'March', averagePrice: 450, recommendation: 'excellent' }
            ],
            marketTrends: { trend: 'stable', confidence: 0.8, timeframe: 'monthly' },
            lastUpdated: new Date()
          }),
          getOptimalBookingWindows: vi.fn().mockReturnValue([
            { hotelId: 'biarritz_palace', hotelName: 'Hôtel du Palais', city: 'biarritz' }
          ]),
          getCuratedHotels: vi.fn().mockReturnValue([
            { hotelId: 'biarritz_palace', name: 'Hôtel du Palais', city: 'biarritz', category: 'palace' },
            { hotelId: 'bordeaux_grand', name: 'Grand Hôtel Bordeaux', city: 'bordeaux', category: 'luxury' }
          ])
        };
      case 'nft-data':
        return {
          getCuratedNFTsData: vi.fn().mockReturnValue({
            collections: [
              { 
                ...
                stats: { floor_price: 2.5, one_day_volume: 100, one_day_change: 5 },
      ...
      case 'altcoin-data':
        return {
          getCuratedAltcoinsData: vi.fn().mockReturnValue({
            coins: [
              { id: 'ethereum', name: 'Ethereum', symbol: 'ETH', price: 3000, change24h: 2.5 }
            ],
            summary: {
      ...
      case 'stock-data':
        return {
          getCuratedStockData: vi.fn().mockReturnValue({
            stocks: [
              { symbol: 'MSTR', name: 'MicroStrategy', price: 500, change24h: 5.2 }
            ],
            summary: {
          ...
          getStockData: vi.fn().mockReturnValue({
            stocks: [
              { symbol: 'MSTR', name: 'MicroStrategy', price: 500, change24h: 5.2, changePercent: 5.2, sector: 'bitcoin-related' }
            ],
            performance: {
              topPerformers: [{ symbol: 'MSTR' }],
          ...
          getBitcoinRelatedStocks: vi.fn().mockReturnValue({
            stocks: [
              { symbol: 'MSTR', name: 'MicroStrategy', price: 500, change24h: 5.2, changePercent: 5.2, sector: 'bitcoin-related' },
              { symbol: 'COIN', name: 'Coinbase', price: 200, change24h: 3.1, changePercent: 3.1, sector: 'bitcoin-related' }
            ],
            summary: {
          ...
          getMag7Performance: vi.fn().mockReturnValue([
            ...
            { symbol: 'GOOGL', name: 'Alphabet', price: 175, changePercent: 1.8 },
            { symbol: 'MSFT', name: 'Microsoft', price: 430, changePercent: 3.5 }
          ])
        };
      case 'lifestyle-data':
        return {
          getWeatherData: vi.fn().mockReturnValue({
            cities: [
              { 
                ...
                weather: { current: { temperature_2m: 22, wind_speed_10m: 12 } },
                airQuality: { current: { pm2_5: 8, uv_index: 5 } },
                marine: { current: { wave_height: 1.5, sea_surface_temperature: 20 } }
              }
            ],
            summary: {
          ...
          getLuxuryHotels: vi.fn().mockReturnValue([
          ...
          getOptimalBookingPeriods: vi.fn().mockResolvedValue([
            { 
              ...
              weatherDuringPeriod: { suitabilityScore: 7, conditions: 'mild' },
...
describe('Providers Integration', () => {
  ...
  describe('Time Provider', () => {
    ...
    it('should provide time context', async () => {
      const result = await timeProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Bitcoin Market Provider', () => {
    ...
    it('should provide Bitcoin market data', async () => {
      const result = await bitcoinMarketProvider.get(mockRuntime, mockMessage, mockState);
    ...
    it('should handle service unavailability gracefully', async () => {
      ...
      const result = await bitcoinMarketProvider.get(mockRuntimeNoService, mockMessage, mockState);
  ...
  describe('Real-Time Data Provider', () => {
    ...
    it('should provide real-time market data', async () => {
      const result = await realTimeDataProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('News Provider', () => {
    ...
    it('should provide news and sentiment data', async () => {
      const result = await newsProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Economic Indicators Provider', () => {
    ...
    it('should provide economic indicators', async () => {
      const result = await economicIndicatorsProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Market Context Provider', () => {
    ...
    it('should provide advanced market analysis', async () => {
      const result = await marketContextProvider.get(mockRuntime, mockMessage, mockState);
    ...
    it('should handle missing Bitcoin price gracefully', async () => {
      ...
      const result = await marketContextProvider.get(mockRuntimeWithFailingService, mockMessage, stateWithoutPrice);
      
      expect(result.text).toBe('Bitcoin price data required for market context analysis');
  ...
  describe('Travel Provider', () => {
    ...
    it('should provide travel booking data', async () => {
      const result = await travelProvider.get(mockRuntime, mockMessage, mockState);
    ...
    it('should handle travel service unavailability gracefully', async () => {
      ...
      const result = await travelProvider.get(mockRuntimeNoService, mockMessage, mockState);
      
      expect(result.text).toBe('Travel booking services temporarily unavailable.');
    ...
    it('should handle no travel data gracefully', async () => {
      ...
      const result = await travelProvider.get(mockRuntimeNoData, mockMessage, mockState);
      
      expect(result.text).toBe('Travel data is being updated. Please try again in a few moments.');
      expect(result.values.travelAvailable).toBe(false);
    ...
    it('should handle travel service errors gracefully', async () => {
      ...
      const result = await travelProvider.get(mockRuntimeError, mockMessage, mockState);
      
      expect(result.text).toBe('Travel booking services encountered an error. Please try again later.');
      expect(result.values.travelAvailable).toBe(false);
  ...
  describe('Altcoin Provider', () => {
    ...
    it('should provide altcoin market data', async () => {
      const result = await altcoinProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Stock Provider', () => {
    ...
    it('should provide stock market data', async () => {
      const result = await stockProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Lifestyle Provider', () => {
    ...
    it('should provide lifestyle and destination data', async () => {
      const result = await lifestyleProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Network Health Provider', () => {
    ...
    it('should provide Bitcoin network health data', async () => {
      const result = await networkHealthProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Opportunity Provider', () => {
    ...
    it('should provide investment opportunity data', async () => {
      const result = await opportunityProvider.get(mockRuntime, mockMessage, mockState);
  ...
  describe('Briefing Provider', () => {
    ...
    it('should provide intelligence briefing data', async () => {
      const result = await briefingProvider.get(mockRuntime, mockMessage, mockState);
