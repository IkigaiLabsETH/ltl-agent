...
describe("BitcoinNetworkService", () => {
    ...
    it("should update comprehensive Bitcoin data successfully", async () => {
      ...
      (global.fetch as any)
        .mockResolvedValueOnce({
          ok: true,
          json: () =>
            Promise.resolve({ bitcoin: { usd: 50000, usd_24h_change: 2.5 } }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: () =>
  ...
  describe("Utility Methods", () => {
    ...
    it("should handle fetch with timeout", async () => {
      ...
      const response = await service["fetchWithTimeout"](
        "https://example.com",
        { timeout: 5000 },
      );
    ...
    it("should handle fetch timeout", async () => {
      ...
      await expect(
        service["fetchWithTimeout"]("https://example.com", { timeout: 50 }),
