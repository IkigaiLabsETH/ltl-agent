...
describe("NFTDataService", () => {
  ...
  describe("Data Update Methods", () => {
    ...
    it("should handle update errors gracefully", async () => {
      ...
      vi.spyOn(service as any, "updateCuratedNFTsData").mockRejectedValue(
        error,
      );
  ...
  describe("Data Fetching", () => {
    it("should fetch curated NFTs data successfully", async () => {
      ...
      vi.spyOn(service as any, "fetchEnhancedCollectionData").mockResolvedValue(
        mockCollectionData,
      );
    ...
    it("should handle fetch errors gracefully", async () => {
      ...
      vi.spyOn(service as any, "fetchEnhancedCollectionData").mockRejectedValue(
        error,
      );
  ...
  describe("Collection Data Processing", () => {
    ...
    it("should determine collection category correctly", () => {
      ...
      expect(
        (service as any).determineCollectionCategory(blueChipCollection),
      ).toBe("blue-chip");

      const pfpCollection = { description: "Profile picture collection" };
      expect((service as any).determineCollectionCategory(pfpCollection)).toBe(
        "pfp",
      );

      const generativeCollection = { description: "Generative art collection" };
      expect(
        (service as any).determineCollectionCategory(generativeCollection),
      ).toBe("generative-art");

      const utilityCollection = {
        description: "Utility and access collection",
      };
      expect(
        (service as any).determineCollectionCategory(utilityCollection),
      ).toBe("utility");

      const defaultCollection = { name: "Random Collection" };
      expect(
        (service as any).determineCollectionCategory(defaultCollection),
      ).toBe("digital-art");
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle individual collection fetch errors", async () => {
      ...
      expect(globalErrorHandler.handleError).toHaveBeenCalledWith(error, {
        component: "NFTDataService",
        operation: "fetchCuratedNFTsData.test-2",
      });
    });
  });
