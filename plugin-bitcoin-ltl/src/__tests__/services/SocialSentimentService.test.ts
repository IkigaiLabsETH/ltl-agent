...
describe("SocialSentimentService", () => {
  ...
  describe("Data Update Methods", () => {
    it("should update social sentiment data successfully", async () => {
      const updateSentimentSpy = vi.spyOn(
        service as any,
    ...
    it("should handle update errors gracefully", async () => {
      ...
      vi.spyOn(service as any, "updateSocialSentiment").mockRejectedValue(
        error,
      );
  ...
  describe("Mention Counting", () => {
    it("should count mentions correctly", () => {
      ...
      const result = (service as any).countMentions(messages, [
        "bitcoin",
        "btc",
      ]);
  ...
  describe("Performance and Reliability", () => {
    ...
    it("should handle large message volumes", () => {
      const largeMessages = Array(10000)
        .fill(null)
        .map((_, i) => `Message ${i} Bitcoin`);
