...
export class TravelDataService extends BaseDataService {
  ...
  private readonly curatedHotels: CuratedHotel[] = [
    ...
    // Monaco Luxury Hotels
    {
      ...
      category: 'palace',
      starRating: 5,
      description: 'Belle Époque palace with Mediterranean gardens, casino proximity',
  ...
  private getSeasonalPriceMultiplier(city: string, month: number): number {
    const events = this.seasonalEvents[city as keyof typeof this.seasonalEvents] || [];
    const event = events.find(e => e.month === month);
    
    if (event) {
      switch (event.impact) {
        case 'extreme': return 3.5;
        case 'very-high': return 2.8;
  ...
  private async analyzeOptimalBookingWindows(): Promise<OptimalBookingWindow[]> {
  ...
  private async analyzeHotelOptimalBooking(hotel: CuratedHotel): Promise<OptimalBookingWindow | null> {
    try {
      ...
      let demandLevel: 'very-low' | 'low' | 'moderate' | 'high' | 'very-high' = 'moderate';
      ...
      const bestAvailabilityDate = hotelRates
        .filter(rate => rate.availableRooms > 5)
        .sort((a, b) => b.availableRooms - a.availableRooms)[0]?.checkIn || bestValueDate;
  ...
  private async generateTravelInsights(): Promise<TravelInsights> {
    ...
    for (const location of this.luxuryLocations) {
      ...
      const cityRates = (this.travelDataCache?.data.currentRates || [])
        .filter(rate => cityHotels.some(h => h.hotelId === rate.hotelId));
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      const avgMultiplier = this.luxuryLocations.reduce((sum, loc) => 
        sum + this.getSeasonalPriceMultiplier(loc.city, month), 0) / this.luxuryLocations.length;
      
      const events = this.luxuryLocations.map(loc => 
  ...
  private getEstimatedOccupancyRate(month: number): number {
    const occupancyRates = {
      1: 0.4, 2: 0.3, 3: 0.5, 4: 0.6, 5: 0.7, 6: 0.8,
      7: 0.9, 8: 0.95, 9: 0.7, 10: 0.6, 11: 0.4, 12: 0.5
    };
    
    return occupancyRates[month as keyof typeof occupancyRates] || 0.6;
  }

  private getMonthRecommendation(multiplier: number): 'excellent' | 'good' | 'fair' | 'avoid' {
