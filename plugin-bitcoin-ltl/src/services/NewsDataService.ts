...
export class NewsDataService extends BaseDataService {
  ...
  private async startRealTimeUpdates(): Promise<void> {
    ...
    this.contextLogger.info(
      `News data updates scheduled every ${updateInterval}ms`,
    );
  }
    ...
    try {
      ...
      for (const category of this.newsCategories) {
        try {
          ...
          this.errorHandler.handleError(error, {
            component: "NewsDataService",
            operation: `fetchCategoryNews.${category.name}`,
          });
        }
      }
  ...
  private calculateNewsSummary(newsItems: NewsItem[]): NewsSummary {
    const positiveArticles = newsItems.filter(
      (item) => item.sentiment === "positive",
    ).length;
    const negativeArticles = newsItems.filter(
      (item) => item.sentiment === "negative",
    ).length;
    const neutralArticles = newsItems.filter(
    ...
    return [
      {
        ...
        summary:
          "Bitcoin demonstrates strong fundamentals despite market fluctuations",
      ...
      },
      {
        ...
        summary:
          "Major financial institutions continue to embrace digital assets",
  ...
  public getStats(): {
    ...
    return {
      ...
      positiveArticles: this.newsItems.filter(
        (item) => item.sentiment === "positive",
      ).length,
      negativeArticles: this.newsItems.filter(
        (item) => item.sentiment === "negative",
      ).length,
      neutralArticles: this.newsItems.filter(
        (item) => item.sentiment === "neutral",
      ).length,
