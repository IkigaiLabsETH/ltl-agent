...
export class PerformanceTrackingService extends BaseDataService {
  ...
  private async createSampleData(): Promise<void> {
    ...
    const samplePredictions = [
      ...
      },
      {
        ...
        asset: "msty",
        prediction:
          "MSTY will generate 20%+ annualized yield through volatility harvesting",
        confidence: 0.7,
        timeframe: "12 months",
        catalysts: [
  ...
  async trackPrediction(content: ContentItem): Promise<void> {
    try {
      if (
        !content.insights?.predictions ||
        content.insights.predictions.length === 0
      ) {
        return;
      }
  ...
  private async extractPrediction(
    ...
    try {
      ...
      const asset =
        (content.metadata.assets && content.metadata.assets[0]) || "unknown";
  ...
  private async evaluateExpiredPrediction(
  ...
  private async updateMetrics(): Promise<void> {
    try {
      ...
      this.metrics = {
        ...
        activePredictions: allPredictions.filter((p) => p.status === "active")
          .length,
  ...
  private calculateAccuracyByAsset(
    ...
    for (const asset of [...new Set(predictions.map((p) => p.asset))]) {
      ...
      if (assetOutcomes.length > 0) {
        accuracyByAsset[asset] =
          assetOutcomes.reduce((sum, o) => sum + o.accuracy, 0) /
  ...
  private calculateAccuracyByTimeframe(
    ...
    for (const timeframe of [...new Set(predictions.map((p) => p.timeframe))]) {
      ...
      if (timeframeOutcomes.length > 0) {
        accuracyByTimeframe[timeframe] =
          timeframeOutcomes.reduce((sum, o) => sum + o.accuracy, 0) /
          timeframeOutcomes.length;
      }
    }
  ...
  private calculateAccuracyBySource(
    predictions: Prediction[],
    outcomes: PredictionOutcome[],
    ...
    for (const source of [...new Set(predictions.map((p) => p.source))]) {
      ...
      if (sourceOutcomes.length > 0) {
        accuracyBySource[source] =
          sourceOutcomes.reduce((sum, o) => sum + o.accuracy, 0) /
  ...
  private calculateProfitabilityMetrics(
    outcomes: PredictionOutcome[],
  ): PerformanceMetrics["profitabilityMetrics"] {
    const profitableOutcomes = outcomes.filter(
      (o) => o.profitability !== undefined,
    );
    ...
    return {
      ...
      averageGain:
        wins.length > 0
          ? wins.reduce((sum, o) => sum + (o.profitability || 0), 0) /
            wins.length
          : 0,
      averageLoss:
        losses.length > 0
          ? losses.reduce((sum, o) => sum + (o.profitability || 0), 0) /
            losses.length
          : 0,
    };
  }
  ...
  private calculateRecentPerformance(
    ...
    return {
      last7Days:
        recent7.length > 0
          ? recent7.reduce((sum, o) => sum + o.accuracy, 0) / recent7.length
          : 0,
      last30Days:
        recent30.length > 0
          ? recent30.reduce((sum, o) => sum + o.accuracy, 0) / recent30.length
          : 0,
      last90Days:
        recent90.length > 0
          ? recent90.reduce((sum, o) => sum + o.accuracy, 0) / recent90.length
          : 0,
    };
  }
  ...
  async formatPerformanceForDelivery(): Promise<ProcessedIntelligence> {
    ...
    const sections = [
      ...
      "",
      "🏆 **Top Performers:**",
      ...report.topPredictions.mostAccurate
        .slice(0, 3)
        .map(
          (outcome) =>
