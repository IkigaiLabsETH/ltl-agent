...
export class OpportunityAlertService extends BaseDataService {
  ...
  private async analyzeContentForOpportunities(
    content: ContentItem,
  ...
  private async evaluateCriteria(
    ...
    if (criteria.conditions.sourceImportance) {
      if (
        content.metadata.importance === criteria.conditions.sourceImportance
      ) {
    ...
    if (criteria.conditions.sentimentThreshold) {
      if (
        content.metadata.sentiment === criteria.conditions.sentimentThreshold
      ) {
    ...
    if (
      content.insights?.marketSignals &&
      content.insights.marketSignals.length > 0
    ) {
      signals.push(
  ...
  private async triggerAlert(opportunity: OpportunityAlert): Promise<void> {
    ...
    this.contextLogger.info(
      `🚨 Opportunity Alert: ${opportunity.asset} - ${opportunity.signal}`,
    );
