...
export class SlackIngestionService extends ContentIngestionService {
  ...
  private async checkChannel(channel: SlackChannelConfig) {
    ...
    try {
      ...
      const newMessages = messages.filter(
        (msg) => new Date(parseFloat(msg.ts) * 1000) > this.lastChecked,
      );
  ...
  private async convertMessagesToContent(
  ...
  private async convertSlackMessageToContent(
    ...
    if (
      message.text.includes("twitter.com") ||
      message.text.includes("x.com")
    ) {
      contentType = "tweet";
    } else if (
      message.text.includes("youtube.com") ||
      message.text.includes("podcast")
    ) {
      contentType = "podcast";
   ...
   */
  async getContentByChannelType(
    type: SlackChannelConfig["type"],
  ): Promise<ContentItem[]> {
