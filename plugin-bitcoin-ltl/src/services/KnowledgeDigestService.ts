...
export class KnowledgeDigestService extends BaseDataService {
  ...
  capabilityDescription =
    "Generates daily knowledge digests from ingested content and research";
  ...
  async updateData(): Promise<void> {
    try {
      ...
      if (
        this.dailyContent.has(today) &&
        this.dailyContent.get(today)!.length >= 10
      ) {
  ...
  private async extractTopTopics(content: ContentItem[]): Promise<
    Array<{
      topic: string;
      relevance: number;
      sources: string[];
  ...
  > {
    ...
    const topicMap: Map<
      string,
      { count: number; sources: Set<string>; insights: string[] }
    > = new Map();
  ...
  private async identifyEmergingTrends(content: ContentItem[]): Promise<
    Array<{
      trend: string;
      confidence: number;
      signals: string[];
  ...
  > {
    ...
    const trends = [
      ...
      },
      {
        ...
        potentialImpact:
          "Short-term opportunity for strategic altcoin positions",
      },
      {
  ...
  private async extractResearchHighlights(content: ContentItem[]): Promise<
    Array<{
      title: string;
      source: string;
      significance: string;
  ...
  private async generateMarketIntelligence(content: ContentItem[]): Promise<
    Array<{
      asset: string;
      prediction: string;
      confidence: number;
    ...
    const intelligence = [
      {
        asset: "Bitcoin",
        prediction:
          "Continued institutional accumulation driving price appreciation",
        confidence: 0.8,
      ...
      },
      {
        asset: "MetaPlanet",
        prediction:
          "Japanese Bitcoin strategy validation could drive further gains",
        confidence: 0.75,
        catalysts: [
          "Regulatory clarity",
      ...
      },
      {
        asset: "MSTY",
        prediction:
          "Volatility harvesting strategy continues to generate yield",
        confidence: 0.7,
        catalysts: [
  ...
  private async analyzePerformance(content: ContentItem[]): Promise<
    Array<{
      prediction: string;
      outcome: string;
      accuracy: number;
  ...
  private async identifyWatchItems(content: ContentItem[]): Promise<
    Array<{
      item: string;
      priority: "low" | "medium" | "high";
      reasoning: string;
