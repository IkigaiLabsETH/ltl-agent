...
const ServiceConfigSchema = z.object({
  ...
  altcoinData: z.object({
    ...
    trackedCoins: z.array(z.string()).default([
      'ethereum', 'chainlink', 'solana', 'cardano', 'polygon'
    ]),
  }).default({}),
  ...
  morningBriefing: z.object({
    ...
    includeSections: z.array(z.string()).default([
      'bitcoin_price', 'thesis_progress', 'market_summary', 'news_highlights'
    ]),
  }).default({}),
  ...
  performanceTracking: z.object({
    ...
    includeMetrics: z.array(z.string()).default([
      'price_performance', 'thesis_progress', 'market_metrics'
    ]),
  }).default({}),
  ...
  knowledgeDigest: z.object({
    ...
    sources: z.array(z.string()).default([
      'bitcoin_magazine', 'coindesk', 'cointelegraph'
    ]),
  }).default({}),
...
export class ConfigurationManager {
   ...
   */
  getServiceConfig<T extends keyof ServiceConfig>(serviceName: T): ServiceConfig[T] {
  ...
  getConfigurationStatus(): {
    ...
    const enabledServices = services.filter(service => this.isServiceEnabled(service as keyof ServiceConfig));
   ...
   */
  private mergeConfigs(...configs: Partial<ServiceConfig>[]): Partial<ServiceConfig> {
    const merged: any = {};
  ...
  private logConfigurationSummary(): void {
    ...
    const enabledServices = Object.keys(this.config)
      .filter(key => key !== 'global' && this.isServiceEnabled(key as keyof ServiceConfig));
 ...
 */
export async function initializeConfigurationManager(runtime: IAgentRuntime): Promise<ConfigurationManager> {
  if (!configurationManager) {
