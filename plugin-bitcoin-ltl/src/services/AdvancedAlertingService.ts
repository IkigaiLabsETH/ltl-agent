...
export class AdvancedAlertingService extends BaseDataService {
    ...
    try {
      const defaultRules: AlertRule[] = [
        {
          ...
          description:
            "Alert when Bitcoin price drops more than 10% in 24 hours",
          ...
          customMessage:
            "Bitcoin has dropped significantly. Consider reviewing your position.",
          metadata: {
            createdBy: "system",
            createdAt: Date.now(),
            updatedAt: Date.now(),
        ...
        },
        {
          ...
          description:
            "Alert when Bitcoin price increases more than 15% in 24 hours",
          category: "price_movement",
          condition: "price_change_percent",
        ...
        },
        {
          ...
          description:
            "Alert when RSI goes below 30 indicating oversold conditions",
        ...
        },
        {
          ...
          description:
            "Alert when RSI goes above 70 indicating overbought conditions",
          category: "technical_breakout",
      ...
      await handleError(
        error instanceof Error
          ? error
          : new Error("Failed to initialize default rules"),
        {
  ...
  private async initializeDefaultChannels(): Promise<void> {
    try {
      const defaultChannels: NotificationChannel[] = [
        {
          ...
          config: {
            ...
            recipients: this.getSetting("ALERT_EMAIL_RECIPIENTS", "")
              .split(",")
              .filter(Boolean),
          },
      ...
      await handleError(
        error instanceof Error
          ? error
          : new Error("Failed to initialize default channels"),
        {
          correlationId: this.correlationId,
          component: "AdvancedAlertingService",
  ...
  private async initializeDefaultTemplates(): Promise<void> {
    try {
      const defaultTemplates: AlertTemplate[] = [
        {
          ...
          messageTemplate:
            "Bitcoin price has {{direction}} by {{change}}% to ${{price}}. This represents a {{magnitude}} movement.",
        ...
        },
        {
          ...
          messageTemplate:
            "Trading volume has increased by {{change}}% to {{volume}}. This may indicate increased market activity.",
        ...
        },
        {
          ...
          messageTemplate:
            "{{indicator}} has reached {{value}}, indicating {{condition}} conditions.",
      ...
      await handleError(
        error instanceof Error
          ? error
          : new Error("Failed to initialize default templates"),
        {
          correlationId: this.correlationId,
          component: "AdvancedAlertingService",
  ...
  private async evaluateAlertRules(): Promise<void> {
    try {
      ...
      await handleError(
        error instanceof Error
          ? error
          : new Error("Alert rule evaluation failed"),
        {
          correlationId: this.correlationId,
          component: "AdvancedAlertingService",
    ...
    try {
      switch (condition) {
        ...
        case "volume_change_percent":
          const volumeChange =
            ((data.volume - data.volume24h) / data.volume24h) * 100;
  ...
  private checkRateLimit(channel: NotificationChannel): boolean {
    ...
    return (
      channel.rateLimit.currentHour < channel.rateLimit.maxPerHour &&
  ...
  private generateAlertTitle(rule: AlertRule, data: any): string {
    ...
    if (template) {
      return this.interpolateTemplate(
        ...
        this.getTemplateVariables(rule, data),
      );
    }
  ...
  private getChange(condition: AlertCondition, data: any): any {
    switch (condition) {
      case "price_change_percent":
        return (((data.price - data.price24h) / data.price24h) * 100).toFixed(
          2,
        );
      case "volume_change_percent":
        return (
          ((data.volume - data.volume24h) / data.volume24h) *
          100
        ).toFixed(2);
      default:
        return null;
