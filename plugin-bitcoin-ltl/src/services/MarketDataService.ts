...
export class MarketDataService extends BaseDataService {
  ...
  private async updateMarketData(): Promise<void> {
    ...
    try {
      const [priceData, globalData, trendingData, exchangeRates] = await Promise.all([
        this.fetchPriceData(),
      ...
      this.marketData = {
        bitcoin: priceData?.bitcoin || { price: null, change24h: null, marketCap: null, volume24h: null, dominance: null },
        ethereum: priceData?.ethereum || { price: null, change24h: null, marketCap: null, volume24h: null },
        global: globalData || { 
          totalMarketCap: null, 
          totalVolume24h: null, 
  ...
  private async fetchPriceData(): Promise<{ bitcoin: any; ethereum: any } | null> {
    try {
  ...
  private async fetchGlobalData(): Promise<any> {
    try {
      ...
      return {
        ...
        marketCapChange24h: data.data?.market_cap_change_percentage_24h_usd || null,
  ...
  public isDataFresh(): boolean {
    ...
    return Date.now() - this.marketData.lastUpdated.getTime() < this.CACHE_DURATION;
  }
} 