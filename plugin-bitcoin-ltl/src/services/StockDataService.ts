...
export class StockDataService extends BaseDataService {
  ...
  capabilityDescription =
    "Provides real-time stock market data for curated equities with performance analysis vs MAG7 and S&P 500";
  ...
  private readonly curatedStocks = [
    // Bitcoin/Crypto Related Stocks
    {
      ...
      sector: "bitcoin-related" as const,
    },
    {
      symbol: "COIN",
  ...
  private isStockCacheValid(): boolean {
    ...
    return (
      Date.now() - this.stockDataCache.timestamp < this.STOCK_CACHE_DURATION
    );
  }
  ...
  private async fetchSingleStockData(
    symbol: string,
    name: string,
    sector: string,
  ...
  private calculatePerformanceMetrics(
    ...
    const safeAverage = (arr: StockData[]): number => {
      ...
      const validPercentages = arr.filter((stock) =>
        isFinite(stock.changePercent),
      );
      if (validPercentages.length === 0) return 0;
      return (
        validPercentages.reduce((sum, stock) => sum + stock.changePercent, 0) /
    ...
    const sp500Performance =
      indices.find((i) => i.symbol === "SPY")?.changePercent || 0;
