...
import { BitcoinETF, ETFFlowData, ETFFlowSummary, ETFHistoricalData, ETFMarketData } from '../types';
...
export class ETFDataService extends BaseDataService {
  ...
  private async getETFFlowSummary(): Promise<ETFFlowSummary> {
    ...
    const totalNetFlow = flowData.reduce((sum: number, flow: ETFFlowData) => sum + flow.inflow, 0);
    const totalInflow = flowData.filter((flow: ETFFlowData) => flow.inflow > 0).reduce((sum: number, flow: ETFFlowData) => sum + flow.inflow, 0);
    const totalOutflow = flowData.filter((flow: ETFFlowData) => flow.inflow < 0).reduce((sum: number, flow: ETFFlowData) => sum + Math.abs(flow.inflow), 0);
    
    const topInflows = flowData.filter((flow: ETFFlowData) => flow.inflow > 0).sort((a: ETFFlowData, b: ETFFlowData) => b.inflow - a.inflow).slice(0, 5);
    const topOutflows = flowData.filter((flow: ETFFlowData) => flow.inflow < 0).sort((a: ETFFlowData, b: ETFFlowData) => a.inflow - b.inflow).slice(0, 5);
    
    return {
      totalNetFlow,
      totalInflow,
      totalOutflow,
      totalVolume: flowData.reduce((sum: number, flow: ETFFlowData) => sum + flow.volume, 0),
      totalBitcoinHoldings: flowData.reduce((sum: number, flow: ETFFlowData) => sum + flow.bitcoinHoldings, 0),
      totalBitcoinValue: flowData.reduce((sum: number, flow: ETFFlowData) => sum + flow.bitcoinValue, 0),
      totalAUM: flowData.reduce((sum: number, flow: ETFFlowData) => sum + flow.bitcoinValue, 0),
      averagePremium: flowData.reduce((sum: number, flow: ETFFlowData) => sum + flow.premium, 0) / flowData.length,
      topInflows,
      topOutflows,
      date: new Date().toISOString().split('T')[0],
      lastUpdated: new Date().toISOString()
   ...
   */
  private calculateMarketMetrics(etfs: BitcoinETF[], flowSummary: ETFFlowSummary): any {
