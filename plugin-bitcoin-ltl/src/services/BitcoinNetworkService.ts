...
export class BitcoinNetworkService extends BaseDataService {
  ...
  private async fetchBlockchainInfoData(): Promise<Partial<BitcoinNetworkData> | null> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Blockchain.info API error: ${response.status} ${response.statusText}`,
        );
      }
  ...
  private async fetchMempoolNetworkData(): Promise<Partial<BitcoinNetworkData> | null> {
    try {
      const [mempoolResponse, feesResponse] = await Promise.all([
        this.fetchWithTimeout(`${this.MEMPOOL_API}/mempool`, {
          timeout: 10000,
        }),
        this.fetchWithTimeout(`${this.MEMPOOL_API}/v1/fees/recommended`, {
          timeout: 10000,
        }),
      ]);
      ...
      const mempoolData =
        (await mempoolResponse.json()) as MempoolSpaceResponse;
  ...
  public isDataFresh(): boolean {
    ...
    const age =
      Date.now() - this.comprehensiveBitcoinData.lastUpdated.getTime();
