...
export class BitcoinTestSuite implements TestSuite {
  ...
  tests = [
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        console.log("🧪 Testing Bitcoin data service...");
        const service = runtime.getService("bitcoin-data"); // Use 'bitcoin-data' service name
        if (!service) {
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (character.name !== "Satoshi") {
          throw new Error(
            `Expected character name 'Satoshi', got '${character.name}'`,
          );
        }
        ...
        if (character.knowledge.length < 10) {
          throw new Error(
            `Expected at least 10 knowledge files, got ${character.knowledge.length}`,
          );
        }

        console.log(
          `Knowledge files configured: ${character.knowledge.length}`,
        );
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        // Check if bitcoin-ltl plugin exists
        const bitcoinPlugin = runtime.plugins.find(
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const enhancedEmbeddingError = ElizaOSErrorHandler.handleCommonErrors(
          embeddingError,
          "test",
        );
        ...
        const enhancedDbError = ElizaOSErrorHandler.handleCommonErrors(
          dbError,
          "test",
        );
        ...
        const enhancedApiError = ElizaOSErrorHandler.handleCommonErrors(
          apiError,
          "test",
        );
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const priceProvider = bitcoinPlugin.providers.find(
        ...
        const thesisProvider = bitcoinPlugin.providers.find(
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (
          bootstrapIndex !== -1 &&
          bootstrapIndex !== pluginNames.length - 1
        ) {
          console.warn(
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const isDbConfigObject = (
          config: any,
        ): config is Record<string, any> => {
        ...
        if (databaseConfig && isDbConfigObject(databaseConfig)) {
          ...
          if (
            databaseConfig.type &&
            !["pglite", "postgresql"].includes(databaseConfig.type)
          ) {
            throw new Error(
