...
export class KnowledgePerformanceMonitor extends Service {
  ...
  get capabilityDescription(): string {
    return 'Knowledge system performance monitoring and metrics tracking';
  }
  ...
  recordSearch(query: string, responseTime: number, resultsCount: number, cacheHit: boolean = false, error?: string): void {
  ...
  getHealthStatus(): { healthy: boolean; issues: string[] } {
    ...
    const daysSinceUpdate = (Date.now() - this.metrics.lastIndexUpdate.getTime()) / (1000 * 60 * 60 * 24);
