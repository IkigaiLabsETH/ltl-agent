...
async function listEnvVars() {
  ...
  console.info(`  Platform: ${base_exports.cyan(envInfo.os.platform)} (${envInfo.os.release})`);
  console.info(`  Architecture: ${base_exports.cyan(envInfo.os.arch)}`);
  console.info(`  CLI Version: ${base_exports.cyan(envInfo.cli.version)}`);
  console.info(
  ...
  console.info(`Path: ${localEnvFilePath ?? path2.join(process.cwd(), ".env")}`);
  if (!localEnvFilePath || !existsSync(localEnvFilePath)) {
    console.info(base_exports.yellow("  No local .env file found"));
    const exampleEnvPath = path2.join(process.cwd(), ".env.example");
    if (existsSync(exampleEnvPath)) {
      console.info(base_exports.red("  \u2716 Missing .env file. Create one with:"));
      console.info(`     ${base_exports.bold(base_exports.green("cp .env.example .env"))}`);
    } else {
      console.info(
        base_exports.red(
    ...
    if (Object.keys(localEnvVars).length === 0) {
      ...
      for (const [key, value] of Object.entries(localEnvVars)) {
        console.info(`  ${base_exports.green(key)}: ${maskedValue(value)}`);
      }
    }
  }
  console.info("\n");
...
async function editEnvVars(scope, fromMainMenu = false, yes = false) {
...
async function resetEnv(yes = false) {
  const elizaDir = path2.join(process.cwd(), ".eliza");
  const cacheDir = path2.join(elizaDir, "cache");
  const localEnvPath = await getLocalEnvPath() ?? path2.join(process.cwd(), ".env");
  ...
  const resetItems = [
    {
      ...
      value: "localEnv",
      description: existsSync(localEnvPath) ? "Reset values in local .env file" : "Local .env file not found, nothing to reset",
  ...
  const validResetItems = resetItems.filter(
    async (item) => item.value === "localEnv" && existsSync(localEnvPath) || item.value === "cache" && existsSync(cacheDir) || item.value === "localDb" && existsSync(localDbDir)
  );
  let selectedValues = [];
  if (yes) {
    selectedValues = validResetItems.map((item) => item.value);
    if (selectedValues.length > 0) {
      console.info(base_exports.bold("The following items will be reset:"));
    ...
    const { selections } = await (0, import_prompts.default)({
      type: "multiselect",
      name: "selections",
      message: base_exports.cyan(base_exports.bold("Select items to reset:")),
      choices: resetItems,
      instructions: false,
